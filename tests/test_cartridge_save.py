...
class TestCartridgeSave(unittest.TestCase):
    def test_has_battery_detects_pokemon_blue_cart_type(self) -> None:
        self.assertTrue(has_battery(0x13))
        self.assertFalse(has_battery(0x12))

    def test_get_save_path_replaces_rom_extension(self) -> None:
    ...
    def test_save_and_load_cartridge_ram(self) -> None:
    ...
    def test_clean_ram_does_not_create_save(self) -> None:
        rom = bytearray(0x8000)
        mbc = MBC3(rom, ram_size=0x8000)
    ...
    def test_force_saves_clean_ram(self) -> None:
    ...
    def test_failed_atomic_replace_preserves_original_and_dirty_state(self) -> None:
        mbc = MBC0(bytearray(0x8000), ram_size=0x0800)
        mbc.ram[0] = 0x42
        mbc.ram_dirty = True
    ...
    def test_loading_enabled_unbanked_ram_refreshes_memory_window(self) -> None:
