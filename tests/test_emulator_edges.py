...
def test_audio_callback_consumes_contiguous_wrapped_partial_and_empty_buffers(
    ...
    apu = APU()
    callback = emulator.make_audio_callback(apu, verbose=True, high_pass=False)
...
def test_audio_callback_high_pass_filter_removes_dc() -> None:
    apu = APU()
    callback = emulator.make_audio_callback(apu)
    ...
    for _ in range(8):
        ...
        apu.buffer_read_pos = 0
        apu.buffer_size = 256
        callback(out, 256, None, None)

    assert abs(out[-1, 0]) < 0.0001
    assert abs(out[-1, 1]) < 0.0001
