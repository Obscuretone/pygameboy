...
from constants import (
    ECHO_OFFSET, BOOT_ROM_SIZE,
    ROM_START, ROM_END, ERAM_START, ERAM_END, VRAM_START, VRAM_END,
    OAM_START, OAM_END, ECHO_START, ECHO_END, IO_START,
    HRAM_START,
    REG_JOYP, REG_SB, REG_SC, REG_DIV, REG_TAC, REG_IF, REG_LY, REG_BOOT,
    REG_NR10, REG_WAVE_RAM_END, REG_LCDC, REG_WX
)
...
class Memory:
    ...
    def _read_echo_ram(self, address: int) -> int:
        return self.memory[address - ECHO_OFFSET]

    def _write_echo_ram(self, address: int, value: int) -> None:
        self.memory[address - ECHO_OFFSET] = value
    ...
    def _read_io_hram(self, address: int) -> int:
        ...
        # IO Registers
        if address == REG_JOYP: return self.joypad.read()
        if address in [REG_SB, REG_SC]: return self.serial.read_byte(address)
        if REG_NR10 <= address <= REG_WAVE_RAM_END: return self.apu.read_byte(address)
    ...
    def _write_io_hram(self, address: int, value: int) -> None:
        ...
        # IO Registers
        if address == REG_JOYP: self.joypad.write(value); return
        if address in [REG_SB, REG_SC]: self.serial.write_byte(address, value); return
        if REG_NR10 <= address <= REG_WAVE_RAM_END: self.apu.write_byte(address, value); return
            
        # Timer / Interrupt Registers (Delegated to CPU usually, but kept here for bus writes)
        if address == REG_DIV: self.memory[address] = 0; return # DIV reset
        if address == REG_TAC: self.memory[address] = value & 0x07; return # TAC
